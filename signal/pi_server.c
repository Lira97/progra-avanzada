...
void attendRequest(int client_fd)
{
    ...
    int chars_read;
    unsigned long int iterations;// variable for the number of iteration
    ...
    test_fds.fd = client_fd;// Polling stdin
    test_fds.events = POLLIN;

    while (current_iteration < iterations)//loop for the pi iterations
    {
        poll_result = poll(&test_fds, 1, timeout);
        // Timeout finished without incidents
        if (poll_result == 0)
        {
          // If it does not detect a singal, the value of pi will be calculate
        ...
        {
          // if the the client is quitting, the program will send the current value of PI
        ...
        else // if (poll_result == -1)  The poll returned an error
        {
