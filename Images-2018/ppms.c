...
void readMatrixFile(char * filename, Matrix * matrix)
{
    FILE * file_ptr = NULL;
...
void getValues(Matrix * matrix, FILE * file_ptr)
{
...
void Filter(ppm_t * image, Matrix * matrix)
{
  int kCenterX = floor(matrix->height / 2);//values for the center of the matrix kernel
  int kCenterY = floor(matrix->width / 2);//values for the center of the matrix kernel

  int col,row,i,j;
  #pragma omp parallel for private(col,row,i,j) shared(image,matrix)//parallel in wich it includes row col and the 2 last valueves from the for
  for ( row = 0; row < image->height; row++ )//loop for the image
  {
    for (  col = 0; col < image->width; col++ )
    {
      ...
      for (  i = -kCenterX; i <= kCenterX; i++ )//loop for the matrix kernel
      {
        for (  j = -kCenterY; j <= kCenterY; j++ )
        {
          if (row + i > 0 && col + j > 0 && col + j < image->width && row + i < image->height)//detect the borders
          {
            int x = i + kCenterX;//index for kernel matrix
            int y = j + kCenterY;//index for kernel matrix
            blue+= image->pixels[row+i][col+j].data[0] * matrix->values[x][y];//sum the result of the multiplication
            green += image->pixels[row+i][col+j].data[1] * matrix->values[x][y];//sum the result of the multiplication
            red += image->pixels[row+i][col+j].data[2] * matrix->values[x][y];//sum the result of the multiplication
            sum += matrix->values[x][y];//sum from all the values of kernel matrix
          }
        }
      }
      if (sum == 0)
      ...
      if(blue <= image->max_value)// if the value of the sum is less than the max_value then it can do the operation
      {
        image->pixels[row][col].data[0]=blue/sum;
      }
      else
      {
        image->pixels[row][col].data[0]= image->max_value;//if not the value should be the max_value
      }
      if(green <= image->max_value)// if the value of the sum is less than the max_value then it can do the operation
      {
      ...
      {
        image->pixels[row][col].data[1]= image->max_value;//if not the value should be the max_value
      }
      if(red <= image->max_value)// if the value of the sum is less than the max_value then it can do the operation
      {
        image->pixels[row][col].data[2]=red/sum;
      }
      else
      {
        image->pixels[row][col].data[2]= image->max_value;//if not the value should be the max_value

      }

      if(blue >= 0)// if the value of the sum is higher than the 0 then it can do the operation
      {
        image->pixels[row][col].data[0]=blue/sum;
      ...
      {
        image->pixels[row][col].data[0]= 0;//if not the value should be the 0
      }
      if(green >= 0)// if the value of the sum is higher than the 0 then it can do the operation
      {
        image->pixels[row][col].data[1]=green/sum;
      ...
      {
        image->pixels[row][col].data[1]= 0;//if not the value should be the 0
      }
      if(red >= 0)// if the value of the sum is higher than the 0 then it can do the operation
      {
        image->pixels[row][col].data[2]=red/sum;
      }
      else
      {
        image->pixels[row][col].data[2]= 0;//if not the value should be the 0
      }
    }
  }
}
