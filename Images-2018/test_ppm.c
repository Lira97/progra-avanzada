...
void imageOperations(char * in_filename, char * out_filename,char * filer_filename, int doFilter,int doNegative, int rotationAngle, int newScale);
...
void parseOptions(int argc, char * argv[])
{
    ...
    if (input_ok && output_ok )
    {
        ...
        imageOperations(in_filename, out_filename, filer_filename,doFilter, doNegative, rotationAngle, newScale);
    }
    else
    {
        usage(argv[0]);
...
void imageOperations(char * in_filename, char * out_filename,char * filer_filename,int doFilter, int doNegative, int rotationAngle, int newScale)
{
    ...
    if (doFilter)
    {
        // enter to the fuction filter to multiply the matrix kernel to the image pixels
        Matrix input_matrix = {0, 0, NULL};//create a matrix kernel
        readMatrixFile(filer_filename, &input_matrix);//read the file for matrix kernel
        Filter(&input_image,&input_matrix);
        freeMemoryMatrix( &input_matrix );//free the memory for the matrix
    }
