...
void * attentionThread(void * arg){
      ...
      if (poll_result == -1)
      {
          ...
          switch(message.op){
              case RECEIVEFILE://receiveFile
                ...
                sprintf(buffer, "%i %ld",  CLIENTUPLOADFILE, numbytes);//send an error
                sendString(data->connection_fd, buffer);
  ...
  //                      int progress = (bytesTransfered*100)/numbytes;
                ...
                numbytes = 0;
                break;
              case SENDFILE://check the balance
                file = readFile(data, message.account,&numbytes, message.optionalFileName, message.optionalFilePath);
                sprintf(buffer, "%i %ld",  CLIENTRECIVEFILE, numbytes);//send an error
                if(fileMaxSize > numbytes){
                  fileMaxSize = numbytes;
                }
                ...
                sendString(data->connection_fd, buffer);
                success = 1;
                read(data->connection_fd, buffer, BUFFER_SIZE);
                ...
                while (bytesTransfered < numbytes){
                  if(success == 0){
                      for(int i = 0; i < fileMaxSize; i++){
                          ...
                          packet[i] = file[bytesTransfered];
                          bytesTransfered++;
                      }
                      success = 1;
                      
                  }
                  write(data->connection_fd, packet, fileMaxSize);
                  read(data->connection_fd, buffer, 1);
                  sscanf(buffer, "%d", &success);
                  printf("\rDownloading File, Please Wait %d bytes OF %ld", bytesTransfered, numbytes);
                  fflush(stdout);      
